...
| `testMocandoProduto`              | Mocking           | Sucesso   | Verifica se os métodos de `Produto` são chamados corretamente ao adicionar produtos a um pedido, simulando a classe `Produto`.                   |
| `testMocandoGerenciadorPedidos`   | Mocking           | Sucesso   | Verifica se os métodos de criação de pedidos e relatório de vendas são chamados corretamente, simulando a classe `GerenciadorPedidos`.           |
